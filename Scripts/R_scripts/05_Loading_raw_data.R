...
# Script Purpose: Load all blast outputs
# Inputs: Protein fasta files from NCBI refseq and blast outputs
# Outputs: N/A
#-----------------------------------------------------------------------------------------------
...
######################################################################
  ...
  load_MAMP_blast_result_files <- list.files(path = './../../Analyses/MAMP_blast/MAMP_blast_outputs/', pattern = ".faa.txt")
  ...
  for (i in 1:length(load_protein_fasta_files)){
    ...
    load_MAMP_blast_result_files[i] <- paste('./../../Analyses/MAMP_blast/MAMP_blast_outputs/', load_MAMP_blast_result_files[i], sep="")
  }
  
  
  # load reference file to determine percent identity of mined MAMPs
  load_reference_MAMPs_fasta <- Biostrings::readAAStringSet(filepath = "./../../Analyses/MAMP_blast//MAMP_database/MAMP_elicitor_list.fasta") #may come with warning messgae..ignore for now
