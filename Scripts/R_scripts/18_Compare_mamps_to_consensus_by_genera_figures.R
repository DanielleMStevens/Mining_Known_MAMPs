...
plot_points_Percent_Ident_of_MAMPs <- function(MAMP_of_interest){
  new_plot <- ggplot(MAMP_of_interest, aes(x = factor(Genera, level = name_list), y = Percent_Identity, colour = Genera, fill = Genera)) +
    ...
    geom_boxplot(color = "black", fill = "white", outlier.alpha = 0, width = 0.19) +
...
#csp22_combined_plots <-
Similarity_csp22 + Copy_number_csp22


ggsave(csp22_combined_plots, filename = "./../Figures/Plot_MAMP_similarity_by_genera_csp22.pdf", device = cairo_pdf, width = 3.7, height = 5, units = "in")
rm(Similarity_csp22)
rm(Copy_number_csp22)
...
Similarity_flg22 <- subset(filtered_hold_MAMP_seqs, MAMP_Hit == 'flg22_consensus')
Similarity_flg22 <- subset(Similarity_flg22, Similarity_flg22$Genera != "Rhodococcus")
Similarity_flg22 <- plot_points_Percent_Ident_of_MAMPs(subset(Similarity_flg22, Similarity_flg22$Genera != "Streptomyces"))
hold_flg22_copy_number_data <- subset(hold_copy_number, MAMP_Hit == "flg22_consensus")
hold_flg22_copy_number_data <- subset(hold_flg22_copy_number_data, hold_flg22_copy_number_data$Genera != "Rhodococcus")
hold_flg22_copy_number_data <- subset(hold_flg22_copy_number_data, hold_flg22_copy_number_data$Genera != "Streptomyces")
...
Similarity_flgII28 <- subset(filtered_hold_MAMP_seqs, MAMP_Hit == 'flgII-28')
Similarity_flgII28 <- plot_points_Percent_Ident_of_MAMPs(subset(Similarity_flgII28, Similarity_flgII28$Genera != "Rhodococcus"))
hold_flgII28_copy_number_data <- subset(hold_copy_number, MAMP_Hit == "flgII-28")
hold_flgII28_copy_number_data <- subset(hold_flgII28_copy_number_data, hold_flgII28_copy_number_data$Genera != "Rhodococcus")
